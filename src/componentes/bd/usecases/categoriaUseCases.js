...
const getCategoriasDB = async () => {
    try {
        const { rows } = await 
        pool.query(`SELECT * FROM categorias ORDER BY nome`);
...
module.exports = { getCategoriasDB, deleteCategoriaDB, addCategoriaDB,
     updateCategoriaDB, getCategoriaPorCodigoDB}